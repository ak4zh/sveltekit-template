<script lang="ts">
    ...
    let { data, disabled, dialogOpen, sheetOpen }: { 
        data: User, disabled: Writable<boolean>, dialogOpen: Writable<boolean>, sheetOpen: Writable<boolean>
    } = $props();
	const form = superForm(data, {
		validators: zodClient(userUpdateByAdminSchema),
        resetForm: false,
        onResult({ result }) {
            if (result.type === 'success') { $dialogOpen = false; $sheetOpen = false; };
        }
	});
	const { form: formData, enhance, submitting, delayed } = form;
    $effect(() => { $disabled = $submitting || $delayed });
