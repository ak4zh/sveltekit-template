...
export const actions = {
	account: async (event) => {
		...
		if (DEMO_ACCOUNT_IDS.includes(event.locals.user?.id as string)) {
	...
	password: async (event) => {
		...
		try {
			...
			if (user) {
				...
				return setError(passwordUpdateForm, m.something_went_wrong());
			}
		} catch (e) {
			console.error(e);
			return setError(passwordUpdateForm, m.flash_password_update_error());
		}
